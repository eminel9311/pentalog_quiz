...
def main
  ...
  data_rentals.each do |rental|
    ...
    commission["insurance_fee"] = (total_price*0.3*0.5).to_i
    commission["assistance_fee"] = (rental_period*100).to_i
    commission["drivy_fee"] = (total_price*0.3 - total_price*0.3*0.5 - rental_period*100).to_i
    obj["id"] = rental["id"]
